...
parser.add_argument("--scale", type=float, default=1.0, help="scale for the radius")
parser.add_argument("--display", action="store_true", help="display batch radius distribution")
args = parser.parse_args()

# define dataset
path_dataset = os.path.join("dataset", "pose_dataset" if args.use_pose else "gt_dataset")
train_dataset = UPBDataset(path_dataset, train=True, scale=args.scale)
...
if __name__ == "__main__":
    ...
    # save to csv
    if args.use_pose:
        path_weights = os.path.join(path_dataset, "weights_%.2f.csv" % (args.scale))
    else:
        path_weights = os.path.join(path_dataset, "weights.csv")
