"""
    File taken from: https://github.com/nemodrive/car_data_collection
...
def get_car_path(r, distance=1., no_points=100, center_x=True, car_t=CAR_T):
    ...
    """
    r_center = r
    r_inner = r_center - car_t / 1.5
    r_outer = r_center + car_t / 1.5
...
def project_points_on_image_space(points_3d):
...
def filter_points(image, points,
                  filter_negative=True,
                  filter_nonmonotonic=True):
    points = project_points_on_image_space(points)
    # print(points)
...
def render_line(image, points, color=None, thickness=None):
...
def render_path_lines(image, radius, camera_position):
    curve_length = 30

    center_color = (0, 255, 0)
    center_width = 4
    side_color = (255, 255, 0)
    side_width = 2

    c, lw, rw = get_car_path(radius, distance=curve_length)
    c = add_3rd_dim(c, camera_position)
    lw = add_3rd_dim(lw, camera_position)
    rw = add_3rd_dim(rw, camera_position)

    c = filter_points(image, c)
    lw = filter_points(image, lw)
    rw = filter_points(image, rw)

    image = render_line(image, c[7:], color=center_color, thickness=center_width)
    image = render_line(image, lw[7:], color=side_color, thickness=side_width)
    image = render_line(image, rw[7:], color=side_color, thickness=side_width)
